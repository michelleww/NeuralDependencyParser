...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        # *** BEGIN YOUR CODE ***
        if self.complete:
            raise ValueError('The parse process is completed.')
        ...
        if transition_id == self.shift_id:
            if (self.next == len(self.sentence)):
                raise ValueError('Has reached the end of the sentence, unable to shift.')
            ...
            if len(self.stack) >= 2:
                ...
                if transition_id == self.left_arc_id:
                    ...
                    idx_dep = self.stack[-2]
                    self.stack.pop(-2)
                else:
                    idx_head = self.stack[-2]
                    idx_dep = self.stack[-1]
                    self.stack.pop(-1)
                self.arcs.append((idx_head,idx_dep, deprel))
                
            else:
                raise ValueError('Invalid stack length')
        # *** END YOUR CODE ***
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        # *** BEGIN YOUR CODE ***
        if len(self.stack) >= 2:
            sec_last = self.stack[-2]
            last = self.stack[-1]
            sec_last_deps = [sec_last_dep for sec_last_dep in get_deps(graph.nodes[sec_last])]
            last_deps = [last_dep for last_dep in get_deps(graph.nodes[last])]

            # second last is a dependent of the last, check for left-arc and shift
            if sec_last in last_deps:
                max_idx = -1
                if len(sec_last_deps) != 0:
                    max_idx = max(sec_last_deps)
                # all dependents of the sec_last has been parsed, can do left-arc transition now     
                if max_idx < self.next:
                    transition_id = self.left_arc_id
                    deprel = graph.nodes[sec_last]['rel']
                # there are dependents of the second last, shift only (should not happen for for projective sentence)
                else:
                    transition_id = self.shift_id

            # last is a dependent of the last, check for right-arc and shift
            elif last in sec_last_deps:
                max_idx = -1
                if len(last_deps) != 0:
                    max_idx = max(last_deps)

                # all dependents of the last has been parsed, can do right-arc transition now
                if max_idx < self.next:
                    transition_id = self.right_arc_id
                    deprel = graph.nodes[last]['rel']
                # there are dependents of the second last, shift only 
                else:
                    transition_id = self.shift_id

            # no dependency relationship between last and second last
            else:
                if self.next >= len(self.sentence):
                    raise ValueError('Has reached the end of the sentence, unable to shift')
                transition_id = self.shift_id
        ...
        # just Root in stack        
        elif len(self.stack) == 1 and self.next < len(self.sentence):
            transition_id = self.shift_id
        # *** END YOUR CODE ***
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i in range(len(minibatch)):
            try:
                transition_pair = td_pairs[i]
                minibatch[i].parse_step(transition_pair[0], transition_pair[1])
