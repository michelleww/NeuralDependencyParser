...
class ParserModel(nn.Module):
    ...
    def create_weights_biases(self):
        ...
        self.W_h = torch.autograd.Variable(he_initializer((N * self.config.embed_size, self.config.hidden_size)), requires_grad=True)
        self.W_o = torch.autograd.Variable(he_initializer((self.config.hidden_size, self.config.n_classes)), requires_grad=True)
        self.b_h = torch.autograd.Variable(torch.zeros(self.config.hidden_size), requires_grad=True)
        self.b_o = torch.autograd.Variable(torch.zeros(self.config.n_classes), requires_grad=True)
        # *** END YOUR CODE ***
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        """
        # *** BEGIN YOUR CODE ***
        look_up_result = torch.matmul(one_hot_float(id_batch,  n_ids), embedding_matrix)
        embedded_batch = torch.reshape(look_up_result, [-1, id_batch.size()[1] * self.config.embed_size])
        # *** END YOUR CODE ***
    ...
    def get_concat_embeddings(self, word_id_batch, tag_id_batch,
        ...
        word_embedded= self.embedding_lookup(word_id_batch, self.config.n_word_ids, self.word_embeddings)
        tag_embedded = self.embedding_lookup(tag_id_batch, self.config.n_tag_ids, self.tag_embeddings)
        deprel_embedded = self.embedding_lookup(deprel_id_batch, self.config.n_deprel_ids, self.deprel_embeddings)
        
        x = torch.cat((word_embedded, tag_embedded, deprel_embedded), 1)        
        # *** END YOUR CODE ***
        return x
    ...
    def add_optimizer(self):
        ...
        self.optimizer = torch.optim.Adam(
            [{'params':self.word_embeddings}, 
            {'params':self.tag_embeddings}, 
            {'params':self.deprel_embeddings}, 
            {'params':self.W_h, 'weight_decay':1e-6}, 
            {'params':self.b_h}, 
            {'params':self.W_o, 'weight_decay':1e-6}, 
            {'params':self.b_o}], lr=self.config.lr)
        # *** END YOUR CODE ***
